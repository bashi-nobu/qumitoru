...
class ImgReader:
    ...
    def saveQuestionnaireTopicImg(self, file_path, user_id):
        download_file_path = s3Lib.download_file(BUCKET_NAME, file_path)
        img = self.read_img(download_file_path)
        img_size_type, height_rate = self.check_pixel_size(img)
        isError, dst = self.inclination_correction(img, img_size_type, height_rate)
        reading_result, point = self.checkReadingQuestionnaireTopics(dst)
        topic_img_list = self.exportQuestionnaireTopicImg(dst, point, self.topic_numbers)
        save_path_list = []
        for img in topic_img_list:
            save_path_list.append(s3Lib.upload_file(BUCKET_NAME, user_id, img))
        return save_path_list, download_file_path

    def uploadEncodeImg(self, bucket_name, downloaded_file_path, s3_file_path):
